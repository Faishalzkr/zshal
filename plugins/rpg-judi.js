...
let handler = async (m, { conn, args, usedPrefix, isOwner }) => {
    ...
    try {
    	let __waktutionskh = (new Date - global.db.data.users[m.sender].lastjudi)
        let _waktutionskh = (5000 - __waktutionskh)
        let waktutionskh = clockString(_waktutionskh)
        if (new Date - global.db.data.users[m.sender].lastjudi > 5000) {
        global.db.data.users[m.sender].lastjudi = new Date * 1
