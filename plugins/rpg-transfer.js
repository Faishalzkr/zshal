            ...
            case 'anvered':
                if (global.db.data.users[m.sender].anvered >= count * 1) {
                    try {
                        ...
                        global.db.data.users[who].anvered += count * 1
                        conn.sendButton(m.chat, `Berhasil mentransfer ${count} ${rpg.emoticon('anvered')}Anvered Crate`.trim(), wm, 'inventory', usedPrefix + 'inv', m)
                    } catch (e) {
                        global.db.data.users[m.sender].anvered += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                    }
                } else conn.sendButton(m.chat, `${rpg.emoticon('anvered')}Anvered crate kamu kamu tidak cukup`.trim(), wm, 'inventory', usedPrefix + 'inv', m)
                break
            default:
                return conn.reply(m.chat, `Gunakan format ${usedPrefix}transfer <type> <jumlah> <@tag>\ncontoh penggunaan: *${usedPrefix}transfer money 100 @${m.sender.split`@`[0]}*\n\n*List yang bisa di transfer*\n${rpg.emoticon('money')}Money\n${rpg.emoticon('potion')}Potion\n${rpg.emoticon('sampah')}Sampah\n${rpg.emoticon('diamond')}Diamond\n${rpg.emoticon('common')}Common\n${rpg.emoticon('uncommon')}Uncommon\n${rpg.emoticon('mythic')}Mythic\n${rpg.emoticon('legendary')}Legendary\n${rpg.emoticon('anvered')}Anvered`.trim(), m, { mentions: m.sender })
        }
    } catch (e) {
